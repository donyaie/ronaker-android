...
class ItemExploreViewModel : BaseViewModel() {
    ...
    fun bind(post: Product, context: DashboardActivity) {
        ...
        if (!(post.price_per_day?.equals(0) ?: (false))) {
            productPrice.value =String.format( "%s%.02f %s",context.getString(R.string.title_curency_symbol) , post.price_per_day , context.getString(
                            R.string.title_per_day))
        } else if (!(post.price_per_week?.equals(0) ?: (false))) {

            productPrice.value =String.format( "%s%.02f %s",context.getString(R.string.title_curency_symbol) ,post.price_per_week , context.getString(
                            R.string.title_per_week))
        } else if (!(post.price_per_month?.equals(0) ?: (false))) {

            productPrice.value =String.format( "%s%.02f %s",context.getString(R.string.title_curency_symbol) , post.price_per_month , context.getString(
